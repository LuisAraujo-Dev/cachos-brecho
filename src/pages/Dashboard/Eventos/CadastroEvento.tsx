...
const CadastroEvento: React.FC<CadastroEventoProps> = ({ onEventCreated }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await postData<Evento, EventoForm>('eventos', formData); 
            
            setMessage({ 
                type: 'success', 
                text: `Bazar "${formData.nome}" agendado para ${formData.data} às ${formData.horario}!` 
            });
    ...
    return (
        <div className="p-6 bg-white shadow-lg rounded-xl border border-[var(--color-salvia)]/30">
            <h2 className="text-2xl font-semibold text-[var(--color-castanho)] mb-4">
                Agendar Novo Bazar/Evento
            </h2>

            {message && (
                <div className={`p-3 mb-4 rounded-md font-medium ${message.type === 'success'
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                    ...
                    className={`w-full py-2 font-bold rounded-md transition duration-300 ${isLoading
                            ...
                            : 'bg-[var(--color-salvia)] text-white hover:bg-[var(--color-salvia)80]'
                        }`}
                    disabled={isLoading}>
